import { NativeMultiAgentRuntime, } from '@getenki/ai';
function printCards(label, cards) {
    console.log(label);
    for (const card of cards) {
        console.log(`- ${card.agentId} (${card.name}) capabilities=${card.capabilities.join(', ')} status=${card.status}`);
    }
}
async function main() {
    ...
    const researchCards = (await runtime.discover('research', "Online" /* JsAgentStatus.Online */));
    printCards('\nResearch-capable agents:', researchCards);
    const response = String(await runtime.process('coordinator', 'basic-ts-multi-agent-session', [
